...
export class StorageServiceShopping {
  ...
  private async init() {
    ...
    const item = await this.storage.get('list-shopping');
  ...
  async setItem(value: any) {
    ...
    await this.storage.set('list-shopping', value);
    this.itemSubject.next(value);
  }

  async getItem() {
    await this.storageReady;
    return await this.storage.get('list-shopping');
  }
}
